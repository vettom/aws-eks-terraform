data "aws_iam_policy_document" "karpenter_assume_role_policy" {
  statement {
    ...
    condition {
      ...
      values   = ["system:serviceaccount:karpenter:karpenter-controller-${var.cluster_name}"]
    }
  ...
  policy = jsonencode(
    {
      ...
      "Statement" : [
        ...
        },
        {
          ...
          "Resource" : "arn:aws:ec2:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:launch-template/*"
        },
        {
          "Sid" : "VisualEditor3",
          "Effect" : "Allow",
