locals {
  appsSecrets_servicaccount = "apps-secrets-${var.cluster_name}"
...
data "aws_iam_policy_document" "appsSecrets_assume_role_policy" {
  statement {
    ...
    condition {
      ...
      values   = ["system:serviceaccount:${local.appsSecrets_namespace}:${local.appsSecrets_servicaccount}"]
    }
...
resource "aws_iam_role" "appsSecrets_iam_role" {
  name               = local.appsSecrets_servicaccount
...
resource "aws_iam_policy" "appsSecrets_read_secrets" {
  name = "appssecrets-readonly-${var.cluster_name}"
  policy = jsonencode(
    {
      "Version" : "2012-10-17",
      "Statement" : [
        {
          ...
          "Resource" : "arn:aws:secretsmanager:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:secret:apps/*"
        }
      ]
    }
  )
